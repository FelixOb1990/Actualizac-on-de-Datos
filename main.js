...
function initProvincias(){
  ['t_provincia','b_provincia'].forEach(id=>{
    ...
    PROVINCIAS.forEach(p=>{const o=document.createElement('option');o.value=p.c;o.textContent=p.n;el.appendChild(o);});
  });
}
...
function cargarCantones(px){
  ...
  if(!pv)return;
  CANTONES.filter(c=>c.p===pv).forEach(c=>{const o=document.createElement('option');o.value=c.c;o.textContent=c.n;cs.appendChild(o);});
  cs.disabled=false;
}
...
function cargarDistritos(px){
  ...
  const ds=document.getElementById(px+'_distrito');
  ds.innerHTML='<option value="">Seleccione distrito...</option>';ds.disabled=true;
  if(!cv)return;
  DISTRITOS.filter(d=>d.c===cv).forEach(d=>{const o=document.createElement('option');o.value=d.n;o.textContent=d.n;ds.appendChild(o);});
  ds.disabled=false;
}
...
function habilitarCampos(){
  ['t_apellido1','t_apellido2','t_nombre1','t_nombre2','t_contacto','t_tel1','t_tel2','t_direccion'].forEach(id=>{
...
async function buscarColaborador(){
  ...
  document.getElementById('sectionTitular').classList.add('hidden');
  try{
    const data=await callFlow(FLOWS.buscarColaborador,{cedula:ced});
    if(!data.items||data.items.length===0){showAlert('alertGlobal','error','No se encontró ningún colaborador con esa cédula.');return;}
    titularItemId=data.items[0].ID;
    llenarTitular(data.items[0]);
    await cargarBeneficiario(ced);
    document.getElementById('sectionTitular').classList.remove('hidden');
    document.getElementById('inputCedula').disabled=true;
...
function llenarTitular(f){
  ...
  const ecMap={'Union Libre':'Unión Libre','Soltero(a)':'Soltero(a)','Casado(a)':'Casado(a)','Separado(a)':'Separado(a)','Divorciado(a)':'Divorciado(a)','Viudo(a)':'Viudo(a)'};
...
async function guardarTitular(){
  ...
  try{
    ...
    const ecVal=(()=>{const m={'Soltero(a)':'Soltero(a)','Casado(a)':'Casado(a)','Unión Libre':'Union Libre','Separado(a)':'Separado(a)','Divorciado(a)':'Divorciado(a)','Viudo(a)':'Viudo(a)'};return m[document.getElementById('t_estadocivil').value]||document.getElementById('t_estadocivil').value;})();
  ...
  }catch(e){showAlert('alertTitular','error','Error: '+e.message);}
  finally{btn.disabled=false;btn.textContent='Guardar Cambios';}
}

async function cargarBeneficiario(ced){
  try{
    const data=await callFlow(FLOWS.gestionarBeneficiario,{accion:'buscar',titular:ced});
    if(data.items&&data.items.length>0){
      benefItemId=data.items[0].ID;
      tieneBeneficiario=true;
      llenarBenef(data.items[0]);
      document.getElementById('benefEmpty').classList.add('hidden');
      document.getElementById('benefForm').classList.remove('hidden');
      document.getElementById('benefActions').style.display='flex';
      document.getElementById('btnEliminarBenef').style.display='inline-block';
    }else{
      tieneBeneficiario=false;benefItemId=null;
      document.getElementById('benefEmpty').classList.remove('hidden');
      document.getElementById('benefForm').classList.add('hidden');
      document.getElementById('benefActions').style.display='none';
    }
  }catch(e){console.error('Error beneficiario:',e);}
}

function llenarBenef(f){
  document.getElementById('b_tipocedula').value=f['TipoCedula']?.Value||'';
  document.getElementById('b_cedula').value=f['Cedula']||'';
  document.getElementById('b_nombre').value=f['Nombre']||'';
  const fn=f['Fechadenacimiento']||'';
  document.getElementById('b_fechanac').value=fn?fn.split('T')[0]:'';
  const ecRawB=f['Estadocivil']?.Value||'';
  const ecMapB={'Union Libre':'Unión Libre','Soltero(a)':'Soltero(a)','Casado(a)':'Casado(a)','Separado(a)':'Separado(a)','Divorciado(a)':'Divorciado(a)','Viudo(a)':'Viudo(a)'};
  document.getElementById('b_estadocivil').value=ecMapB[ecRawB]||ecRawB;
  document.getElementById('b_parentesco').value=f['Parentesco']?.Value||'';
  document.getElementById('b_genero').value=f['Genero']?.Value||'';
  document.getElementById('b_correo').value=f['Correoelectronico']||'';
...
function mostrarFormBenef(){
  ...
  document.getElementById('benefForm').classList.remove('hidden');
  document.getElementById('benefActions').style.display='flex';
  document.getElementById('btnEliminarBenef').style.display='none';
}

function cancelarBenef(){
...
async function guardarBenef(){
  const btn=document.getElementById('btnGuardarBenef');
  btn.disabled=true;btn.textContent='Guardando...';hideAlert('alertBenef');
  try{
    const pe=document.getElementById('b_provincia');
    const ce=document.getElementById('b_canton');
    const ecValB=(()=>{const m={'Soltero(a)':'Soltero(a)','Casado(a)':'Casado(a)','Unión Libre':'Union Libre','Separado(a)':'Separado(a)','Divorciado(a)':'Divorciado(a)','Viudo(a)':'Viudo(a)'};return m[document.getElementById('b_estadocivil').value]||document.getElementById('b_estadocivil').value;})();
    const payload={
      accion:tieneBeneficiario?'editar':'crear',
      titular:document.getElementById('t_cedula').value,
      itemId:benefItemId||0,
      TipoCedula:document.getElementById('b_tipocedula').value,
      Cedula:document.getElementById('b_cedula').value,
      Nombre:document.getElementById('b_nombre').value,
      FechaNac:document.getElementById('b_fechanac').value,
      EstadoCivil:ecValB,
      Parentesco:document.getElementById('b_parentesco').value,
      Genero:document.getElementById('b_genero').value,
      Correo:document.getElementById('b_correo').value,
      Provincia:pe.options[pe.selectedIndex]?.text||'',
      Canton:ce.options[ce.selectedIndex]?.text||'',
      Distrito:document.getElementById('b_distrito').value
    };
    const res=await callFlow(FLOWS.gestionarBeneficiario,payload);
    if(!tieneBeneficiario&&res.id){benefItemId=res.id;tieneBeneficiario=true;}
    document.getElementById('btnEliminarBenef').style.display='inline-block';
    showAlert('alertBenef','success','✓ Beneficiario guardado correctamente.');
  }catch(e){showAlert('alertBenef','error','Error: '+e.message);}
  finally{btn.disabled=false;btn.textContent='Guardar Beneficiario';}
}

function confirmarEliminarBenef(){document.getElementById('modalEliminar').classList.add('show');}
function cerrarModal(){document.getElementById('modalEliminar').classList.remove('show');}

async function eliminarBenef(){
  cerrarModal();
  try{
    await callFlow(FLOWS.gestionarBeneficiario,{accion:'eliminar',itemId:benefItemId});
    tieneBeneficiario=false;benefItemId=null;
    ['b_tipocedula','b_cedula','b_nombre','b_fechanac','b_estadocivil','b_parentesco','b_correo'].forEach(id=>document.getElementById(id).value='');
    document.getElementById('b_provincia').value='';
    document.getElementById('b_canton').innerHTML='<option value="">Seleccione provincia primero</option>';
    document.getElementById('b_canton').disabled=true;
    document.getElementById('b_distrito').innerHTML='<option value="">Seleccione cantón primero</option>';
    document.getElementById('b_distrito').disabled=true;
    document.getElementById('benefEmpty').classList.remove('hidden');
    document.getElementById('benefForm').classList.add('hidden');
    document.getElementById('benefActions').style.display='none';
    showAlert('alertBenef','success','✓ Beneficiario eliminado.');
  }catch(e){showAlert('alertBenef','error','Error: '+e.message);}
}
...
function limpiarFormulario(){
  ...
  titularItemId=null;benefItemId=null;tieneBeneficiario=false;
  ['t_apellido1','t_apellido2','t_nombre1','t_nombre2','t_contacto','t_tel1','t_tel2','t_direccion'].forEach(id=>{
    const el=document.getElementById(id);if(el){el.value='';el.disabled=true;}
  });
  ['t_genero','t_estadocivil'].forEach(id=>{
    const el=document.getElementById(id);if(el){el.value='';el.disabled=true;}
  });
  document.getElementById('t_provincia').value='';
  document.getElementById('t_provincia').disabled=true;
  document.getElementById('t_canton').innerHTML='<option value="">Seleccione provincia primero</option>';
  document.getElementById('t_canton').disabled=true;
  document.getElementById('t_distrito').innerHTML='<option value="">Seleccione cantón primero</option>';
  document.getElementById('t_distrito').disabled=true;
}
