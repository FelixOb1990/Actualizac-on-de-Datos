/**
 * geo-data.js — Datos y helpers compartidos de Provincia/Cantón/Distrito
 * de Costa Rica. Usado por colaborador-app.js (Mi Perfil) y
 * colaboradores-admin.js (Administrar Colaboradores).
 *
 * IMPORTANTE: este script debe cargarse ANTES que cualquier script que
 * lo use (ver router.js, donde 'script' es un array y este archivo va
 * primero). No está envuelto en IIFE a propósito, para que sus
 * const/function queden visibles en el scope global del documento.
 */
...
// ── Helpers de Provincia/Cantón/Distrito ────────────────────────
// 'px' es el prefijo de los IDs en el HTML (ej: 't' para t_provincia,
// 'a' para a_provincia).

function initProvincias(px) {
  const el = document.getElementById(px + '_provincia');
  if (!el) return;
